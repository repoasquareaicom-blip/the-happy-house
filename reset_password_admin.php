...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($password !== $confirm_password) {
        ...
        try {
            ...
            $admin_email = $subscription->validate_password_reset_admin_token($token); // user_id = mail id

            if ($admin_email) {
                $result = $subscription->update_password_admin($admin_email,$hashed_password);
                $success_message = "Password reset successful. You can now <a href='admin_login.php'>log in</a>.$result";
            } else {
                $error_message = "Invalid or expired token. Click <a href='admin_login.php'>here</a> to login ";
    ...
    try {
        $school_admin_email = $subscription->validate_password_reset_admin_token($token);
