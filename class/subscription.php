...
class Subscription{
	...
	public function update_subscription($name, $email, $school_name, $school_admin_email, $password, $subscription_id, $subscription_start, $subscription_end, $cancel_at, $cancel_at_period_end, $status){
		
		$cancel_at_upate ="cancel_at=FROM_UNIXTIME('$cancel_at'),";
		
    	if($cancel_at_period_end == 0){
		  $cancel_at_upate = "cancel_at=null,";    
		}
	...
	public function get_admin_user_by_email($email){
		$_query = "select * from admin_users where email_id = '$email'";
		return $this->data->getData($_query);
	}
	...
	{
		$_query = "INSERT INTO password_reset_tokens (email, token, expiry_time, user_type) 
		VALUES ('$email', '$token', '$expiry_time', '$user_type') 
		ON DUPLICATE KEY UPDATE 
		token = '$token', expiry_time = '$expiry_time'";
                      
		file_put_contents('logs.txt', date("F j, Y, g:i a"). " Method: save_password_reset_token - DB Trans:, Page: Subscription.php, Message: $_query\n", FILE_APPEND);	
...
public function validate_password_reset_token($token)
{
    try {
		...
		try {
                ...
                file_put_contents('logs.txt', date("F j, Y, g:i a"). " Method: Reset Password Validate Token - DB Trans:, Page: reset_password.php, Value: $email - $expiry_time \n", FILE_APPEND);	
        ...
        if (!empty($email)) {
            // file_put_contents('logs.txt', date("F j, Y, g:i a"). " Method: Reset Password Validate Token - DB Trans:, Page: reset_password.php, Value 2: $email - $expiry_time \n", FILE_APPEND);	
            // Check if the token is expired
            if (time() > date('Y-m-d H:i:s', $expiry_time)) {
                // Token has expired
                return false;
            }
...
public function update_password_school_admin($email, $password) 
{
	...
	file_put_contents('logs.txt', date("F j, Y, g:i a"). " Method: School admin password update - DB Trans:, Page: Subscription.php, Query: $_query", FILE_APPEND);	
	$_status = $this->data->execute($_query);
...
public function validate_password_reset_admin_token($token)
{
    try {
        // Prepare the query to find the user based on the token
        $_query = "SELECT   email, expiry_time FROM password_reset_tokens WHERE token = '$token' and user_type='A'";
		file_put_contents('logs.txt', date("F j, Y, g:i a"). " Method: Reset Password Admin Validate Token - DB Trans:, Page: reset_password.php, Query: $_query\n", FILE_APPEND);	
		...
		try {
				...
				file_put_contents('logs.txt', date("F j, Y, g:i a"). " Method: Reset Password Admin Validate Token - DB Trans:, Page: reset_password.php, $email - $expiry_time \n", FILE_APPEND);	
...
public function update_password_admin($email, $password) 
{
	$_query = "UPDATE admin_users SET password = '$password' WHERE email_id = '$email'";			
	file_put_contents('logs.txt', date("F j, Y, g:i a"). " Method: admin password update - DB Trans:, Page: Subscription.php, Query: $_query - $email", FILE_APPEND);	
	$_status = $this->data->execute($_query);
